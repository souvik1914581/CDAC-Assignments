...
struct device_struct my_dev[DEVCOUNT];
...
int open_func(struct inode *inodp,struct file *filep)
{
	...
	struct device_struct *dev = container_of(inodp->i_cdev,struct device_struct,_cdev);
...
/*ssize_t read_func(struct file *filep,char __user *ubuff,size_t count,loff_t *oft)
{
	...
	struct device_struct *dev = filep->private_data;
...
static __init int init_func(void)
{
	...
	if(ret)
	{
		printk("Error registering device_struct\n");
		return -1;
	}
	ret = gpio_is_valid(LED_PIN);
	if(ret<0)
	{
		printk("Error allocating gpio\n");
	}
	
	ret = gpio_request(LED_PIN,"sys_115");
	if(ret < 0)
	{
