...
/*label:*/	
	...
	while( CIRC_CNT(cbuf.head,cbuf.tail,SIZE) == 0);	//block read while kernel buffer is empty
	for(i=0;i<m;i++)
	{
		...
		if(CIRC_CNT(cbuf.head,cbuf.tail,SIZE) >= 1)
		{
...
ssize_t write_func(struct file *filep,const char __user *ubuff,size_t count,loff_t *oft)
{
	...
	while(	CIRC_CNT(cbuf.head,cbuf.tail,SIZE) > 0);	//wait while buffer is not cleared in read func
	for(i=0;i<count;i++)
	{
